...
def generate_events(data):
    ...
    while i <= len(data) - 1:
        yield (f"Event {i + 1}: Player {data[i]['player']} "
               f"(level {data[i]['data']['level']}) {data[i]['event_type']}")
        i += 1
...
def stream_analytics():
    ...
    treasure_events = 0
    while i <= len(data) - 1:
        if data[i]['data']['level'] >= 10:
            high_level_player += 1
        if data[i]['event_type'] == 'treasure':
            treasure_events += 1
        if data[i]['event_type'] == 'level_up':
            level_up_events += 1
        i += 1
    print("\n=== Stream Analytics ===")
...
def generator_demonstration():
    ...
    for i in range(10):
        print(next(fibonacci_number), end='')
        if i < 9:
            print(", ", end='')
        else:
            print()
